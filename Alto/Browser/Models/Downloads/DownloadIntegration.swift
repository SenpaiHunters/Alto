...
public class DownloadIntegration: NSObject {
    ...
    public static func shouldTriggerDownload(for response: HTTPURLResponse) -> Bool {
        ...
        if let contentType = response.allHeaderFields["Content-Type"] as? String {
            let cleanContentType = contentType.lowercased().components(separatedBy: ";").first?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        ...
        if let url = response.url {
            ...
            if pathExtension.isEmpty && !url.lastPathComponent.isEmpty {
                ...
                if lastComponent.contains(where: { $0.isNumber }) || 
...
extension WebPage {
    ...
    @MainActor public func webView(_ webView: WKWebView, decidePolicyFor navigationResponse: WKNavigationResponse, decisionHandler: @escaping (WKNavigationResponsePolicy) -> Void) {
